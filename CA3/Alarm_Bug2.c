// Name: Niki Mardari
// Purpose: Show a false alarm caused by a shared-data race between the INT0 ISR and the main loop. Show that global variables still don't make avr code atmoic on the assembly instruction level.
...
int main(void)
{
    ...
    while (1)
    {
		...
		//if DEBUG is defined in compile command then ignore the delays for gdb debugging
		#ifndef DEBUG
...
Normal build: make or make normal
Debug build (with DEBUG): make debug
Assembly listing: make asm
Flash safely (depends on Alarm_Bug2.hex): make flash
Read fuses: make check_fuses
Enable DWEN: make enable_dwen
Disable DWEN: make disable_dwen
Start avarice: make avarice
Start gdb: make gdb
Clean: make clean
